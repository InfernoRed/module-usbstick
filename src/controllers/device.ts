...
export class UsbDeviceInfo {
    ...
    constructor(id: number, description: string, device: string, path: string, mountpoints: string[]) {
      this.id = id
...
const _getBlockDevice = async (devicePath: string): Promise<BlockDeviceInfo[]> => {
  return new Promise(async resolve => {
    exec(`lsblk ${devicePath} -J`, (_exc: ExecException | null, stdout: string) => {
...
const _getUsbDrives = async (): Promise<UsbDeviceInfo[]> => {
    return new Promise(async resolve => {
      ...
      drives.forEach((drive: any) => {
        if (drive.isRemovable && drive.isUSB && !drive.isSystem) {
          ...
          let driveInfo = new UsbDeviceInfo(availableDrives.length, drive.description, drive.device, drive.devicePath, mountpoints)
  ...
  const _getUsbDrive = async (deviceId: number): Promise<UsbDeviceInfo | null> => {
    return new Promise(async resolve => {
  ...
  const _mountUsbDrive = async (device: string, label: string): Promise<string | ExecException | null> => {
    return new Promise(async resolve => {
      exec(`pmount ${device} ${label}`, (error: ExecException | null, stdout: string) => {
        if (error) {
          resolve(error)
          return
        }
  ...
  const _unmountUsbDrive = async (device: string): Promise<string | ExecException | null> => {
    return new Promise(async resolve => {
...
export async function setup(app: Application) {
    ...
    app.post('/usb/:deviceId/mount', async (_request: Request, response: Response) => {

      let device = await _getUsbDrive(_request.params.deviceId)
      if (!device) {
        response.json({success: false, mnessage: `could not find device ${_request.params.deviceId}`})
        return
      }
      ...
      if (blockDevices.length <= 0 || !blockDevices[0]) {
        response.json({success: false, message: `could not find block device ${device.device}`})
        return
      }
      ...
      if (blockDevices[0].type === 'part') {
        ...
        if (blockDevices[0].children && blockDevices[0].children[0]) {
          ...
          response.json({success: false, message: `could not find partition for block device ${device.device}`})
          return
        }
      }

      var label: string =  _request.get("label") || _request.query.label || `usb_${new Date().getTime()}`
    ...
    app.post('/usb/:deviceId/unmount', async (_request: Request, response: Response) => {
      let device = await _getUsbDrive(_request.params.deviceId)
      if (!device) {
        response.json({success: false, mnessage: `could not find device ${_request.params.deviceId}`})
        return
